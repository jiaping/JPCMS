...
{
    [ControlGroupDescription(Label = "网页头部", Icon = "网页头部", Description = "网页头部", DefaultType = "Header.LogoFlashWithTopItem")]
    [ControlDescription("带Logo带搜索带快捷导航(头)")]
    public partial class Header_LogoFlashWithTopItem : BaseControl
    {
        /// <summary>
        /// 自定义Css类名称
        /// </summary>
        [Parameter(Title = "自定义Css类名称", Type = "String", DefaultValue = "Header_SingleFlash")]
        ...
        public string ChannelUrl
        {
            get
            {
                if (string.IsNullOrEmpty(channelUrl))
                {
                    ...
                    if (string.IsNullOrEmpty(channelID) || channelID.Equals(We7Helper.EmptyGUID))
                        channelUrl = "/";
                    else
                    {
