...
add rePosition option support for jQuery.toolTip

增加自定义 tip 内容容器（而非将整个 tip 的 innerHTML 覆盖掉）
function z(x){var a=x.find("[rel='tipcontent']");return a.length ? a : x}
并相应地作了更改（在 show 方法内将原有 tip.append(title) 位置作了更改，改成了 z(h).empty().append(k) 来实现更改 提示）
...
validator:
 ...
 function is_we7date(dom){return !!$(dom).attr('we7date')} 是否为 date 输入控件
 由于在 jQuery.tools 框架多处有判断当前 input 对象是否为日期输入控件并对执行分支进行处理的需求，但内建的判断是错误的（使用 $().is(":date") 判断出错）

modified the error notice in validator
...
去掉了最后部分的 form 检查（支持所有元素），并对 input 元素作了特殊处理

在 validator 中添加了自定义验证（validator-custom）
添加了 Ajax 验证基础设施
*/